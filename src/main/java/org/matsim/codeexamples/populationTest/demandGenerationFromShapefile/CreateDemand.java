package org.matsim.codeexamples.populationTest.demandGenerationFromShapefile;
...
class CreateDemand {
	...
	private static final String REGION_KEY = "Id";
	...
	CreateDemand() {
		...
		Path sampleFolder = Paths.get("scenarios/sampleScenario/shape");

		this.innerRegionCommuterStatistic = sampleFolder.resolve("saigon-commuters.csv");

		regions = ShapeFileReader.getAllFeatures(sampleFolder.resolve("landuse.shp").toString()).stream()
		...
		for (SimpleFeature feature : ShapeFileReader.getAllFeatures(sampleFolder.resolve("buildings.shp").toString())) {
